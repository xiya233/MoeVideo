ALTER TABLE video_import_items RENAME TO video_import_items_old;
ALTER TABLE video_import_jobs RENAME TO video_import_jobs_old;

CREATE TABLE IF NOT EXISTS video_import_jobs (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    source_type TEXT NOT NULL CHECK (source_type IN ('torrent', 'url')),
...
SELECT
    id, job_id, file_index, file_path, file_size_bytes, selected, status,
    error_message, media_object_id, video_id, created_at, updated_at
FROM video_import_items_old;

DROP TABLE video_import_items_old;
DROP TABLE video_import_jobs_old;
